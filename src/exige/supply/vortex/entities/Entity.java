...
import exige.supply.vortex.engine.physics.Collisions.EntityCollision;
import exige.supply.vortex.engine.physics.Collisions.TileCollision;
    ...
    protected Collision calculateCollision(double xPos, double yPos) {
        ...
        for (int corner = 0; corner < 4; corner++) { // Check all 4 corners
            ...
            for (int i = 0; i < level.getEntities().size(); i++) {
                if (level.getEntities().get(i) == this) continue;
                boolean xPixelCollide = (level.getEntities().get(i).getX() >= xC - xC_Hitbox) && (level.getEntities().get(i).getX() <= xC);
                boolean yPixelCollide = (level.getEntities().get(i).getY() >= yC - yC_Hitbox) && (level.getEntities().get(i).getY() <= yC);
                if (xPixelCollide && yPixelCollide) {
                    col = new EntityCollision(level.getEntities().get(i)); // Set Collision type to Entity Collision, set collide to true
                }
            }
